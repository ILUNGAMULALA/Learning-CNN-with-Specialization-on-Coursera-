...
from tensorflow.keras import backend as K  # Use tensorflow.keras if using TensorFlow 2.x
...
def get_weighted_loss(pos_weights, neg_weights, epsilon=1e-7):
    ...
    pos_weights = K.constant(pos_weights, dtype=K.floatx())
    neg_weights = K.constant(neg_weights, dtype=K.floatx())
    ...
    def weighted_loss(y_true, y_pred):
        ...
        ### START CODE HERE ###
        num_classes = K.int_shape(pos_weights)[0]  # Get the number of classes
        for i in range(num_classes):
            # Positive and negative loss for class i
            ...
            # Add the average loss for class i
